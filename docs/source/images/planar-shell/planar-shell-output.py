...
ax.annotate('LD-Edge-2', xy=(x[1], y[0]+0.9*h), xycoords='data',
            ...
            arrowprops=dict(arrowstyle="->"), **calibri_font)
td_edge_1 = ax.plot((x[0], x[1]), (y[0], y[0]), linewidth=3, c='m')
ax.annotate('TD-Edge-1', xy=(x[0]+0.1*w, y[0]), xycoords='data',
            xytext=(-45,-30), textcoords='offset points',
            arrowprops=dict(arrowstyle="->"), **calibri_font)
td_edge_2 = ax.plot((x[0], x[1]), (y[1], y[1]), linewidth=3, c='m')
top_annotation = ax.annotate('TD-Edge-2', xy=(x[0]+0.1*w, y[1]), xycoords='data',
                             xytext=(-45,30), textcoords='offset points',
                             arrowprops=dict(arrowstyle="->"), **calibri_font)

# Create and annotate the TD-Midpoints.
rp = ax.scatter( [(x[0]+x[1])/2]*2, y, c='b', marker='o')
ax.annotate('TD-Midpoint-1', xy=((x[0]+x[1])/2, y[0]), xycoords='data',
            xytext=(20,-30), textcoords='offset points',
            arrowprops=dict(arrowstyle="->"), **calibri_font)
ax.annotate('TD-Midpoint-2', xy=((x[0]+x[1])/2, y[1]), xycoords='data',
            xytext=(20,30), textcoords='offset points',
            arrowprops=dict(arrowstyle="->"), **calibri_font)
...
plt.xlim([-0.05, 1.05])
plt.ylim([-0.05, 1.10*top_annotation.xy[1]])
